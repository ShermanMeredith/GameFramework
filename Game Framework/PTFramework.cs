...
namespace PlayTable
{
    ...
    public static class PTFramework
    {
        ...
        public static IEnumerator WiggleCoroutine(this Transform trans)
        {
            for (int i = 0; i < 3; ++i)
            {
                trans.Rotate(0, 0, -5);
        ...
        public static IEnumerator BuzzCoroutine(this Transform trans, float intensity = 0.2f, int times = 3, Vector3 buzzDirection = Vector3.right, float timer = PT.DefaultTimer)
        {
            Vector3 startLocalPos = trans.localPosition;
            buzzDirection *= intensity;
            float buzzTime = timer / (times * 2 + 1);
            for (int i = 0; i < times; ++i)
            {
                yield return trans.SetLocalPositionCoroutine(new Vector3(startLocalPos.x + buzzDirection.x, startLocalPos.y + buzzDirection.y, startLocalPos.z + buzzDirection.z), buzzTime);
                yield return trans.SetLocalPositionCoroutine(new Vector3(startLocalPos.x - buzzDirection.x, startLocalPos.y - buzzDirection.y, startLocalPos.z + buzzDirection.z), buzzTime);
            }
