...
public class PTContainerZone : PTZone
{
        ...
        if (swapEnabled)
        {
            OnDropped += (PTTouchFollower droppedObj) =>
            {
                if (Count > 0)
                {
                    ...
                    {
                        newZoneForCurrentObject.Add(Get(0));
                    }
                }
    ...
    public override bool Accepts(Transform obj)
    {
        if (swapEnabled)
        {
            ...
            foreach (GameObject acceptable in acceptedObjectsWhenDropped)
            {
                if (obj.tag == acceptable.tag)
                {
                    canAccept = true;
                    break;
                }
    ...
    public override IEnumerator AddCoroutine(Component component, int siblingIndex, float timer)
    {
        if (component != null)
        {
            ...
            if (fromParent != null && fromParent.GetComponentInParent<PTZone>() != null && fromParent.GetComponentInParent<PTZone>().OnRemoved != null)
            {
